...
const Artisans = () => {
  ...
  const getPlaceholderImage = (profession = '') => {
    ...
    if (job.includes('paint')) return 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=150';
    if (job.includes('plumb')) return 'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?q=80&w=150';
    ...
    const fetchArtisans = async () => {
      try {
        ...
        setArtisans([
          ...
          { _id: '2', name: 'Alex emmanuel', profession: 'plumber', ratings: 5 },
  ...
  const handleViewProfile = (artisan) => {
    ...
    if (lowerName.includes('ubani')) {
      ...
      navigate('/service'); // Default profile dashboard
    }
  };
        ...
        {artisans.slice(0,3).map((artisan) => (
          ...
          <div key={artisan._id} className="artisan-profile-card">
            ...
            <div className="card-top-row">
              <img 
                src={artisan.imageUrl || getPlaceholderImage(artisan.profession)} 
                alt={artisan.name} 
                className="artisan-avatar" 
              />
              {/* Uses text-transform helper inline to auto-capitalize the backend words */}
              <span className="artisan-profession" style={{ textTransform: 'capitalize' }}>
                {artisan.profession === 'furniture' ? 'Carpenter' : artisan.profession}
              </span>
            </div>

            {/* Middle body row */}
            <div className="card-middle-row">
              <h3 className="artisan-name" style={{ textTransform: 'capitalize' }}>
                {artisan.name}
              </h3>
              <div className="artisan-stars">
                {[...Array(5)].map((_, index) => {
                  // Fallback to 5 stars if new accounts have a zero baseline rating as seen in mock data
                  const currentRating = artisan.ratings === 0 ? 5 : artisan.ratings;
                  return (
                    <FaStar 
                      key={index} 
                      className={index < currentRating ? "star active-star" : "star inactive-star"} 
                    />
                  );
                })}
              </div>
